...
# log4js.configure({appenders: [{type: "console"}], levels: {"[all]" : "INFO"}})
#
# logger = log4js.getLogger 'test/basic_decryption_test'

SecureStreams = require('../src/index.coffee')

describe 'SecureStreams.Decrypter', () ->

  private_key = fs.readFileSync('test/testkey.pem', "utf8")

  it 'should decrypt a small string in multiple blocks', (done) ->
    ...
    input.pipe(dec).pipe concat (data) ->
      expect(data.toString('utf8')).to.equal('your text here')
      done()


  it 'should decrypt a small string in a single large block', (done) ->
